...
export default async function ExpensesPage(props: PageProps<"/transactions/expenses">) {
  ...
  return (
    <div className="w-full px-4 py-4 sm:px-6">
      ...
      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          <h1 className="text-[18px] font-semibold tracking-tight">Expenses</h1>
          <p className="mt-0.5 text-[13px] text-muted-foreground">
            All outgoing funds and spending across your organization.
          </p>
        </div>
      ...
      <div className="rounded-[6px] border border-border bg-surface overflow-hidden">
        ...
        <div className="flex border-b border-border px-4 gap-1 pt-1">
          ...
          <Link href="/transactions/income" className="px-3 py-2 text-[13px] text-muted-foreground hover:text-foreground transition-colors">
            Income
          </Link>
            ...
            <div className="flex items-center gap-1">
              {page > 1 && (
                <Button asChild variant="ghost" size="sm">
                  <Link href={`/transactions/expenses?page=${page - 1}`}>&larr;</Link>
                </Button>
              )}
              {[1, 2, 3, 4, 5].filter(p => p <= totalPages).map((p) => (
                <Button key={p} asChild={p !== page} variant={p === page ? "default" : "ghost"} size="sm">
                  {p === page ? <span>{String(p)}</span> : <Link href={`/transactions/expenses?page=${p}`}>{p}</Link>}
                </Button>
              ))}
              {totalPages > 5 && <span className="px-1 text-muted-foreground text-[12px]">...</span>}
              {totalPages > 5 && (
                <Button asChild variant="ghost" size="sm">
                  <Link href={`/transactions/expenses?page=${totalPages}`}>{totalPages}</Link>
                </Button>
              )}
              {page < totalPages && (
                <Button asChild variant="ghost" size="sm">
                  <Link href={`/transactions/expenses?page=${page + 1}`}>&rarr;</Link>
                </Button>
              )}
            </div>
          )}

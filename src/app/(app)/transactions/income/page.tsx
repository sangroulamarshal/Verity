...
export default async function IncomePage(props: PageProps<"/transactions/income">) {
    ...
    <div className="w-full px-4 py-4 sm:px-6">
      ...
      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-[13px] text-muted-foreground">
            All incoming funds and revenue across your organization.
          </p>
        </div>
        <div className="flex items-center gap-2">
          {canEdit && (
            <TransactionDialog
              ...
              trigger={<Button size="sm">+ Add Income</Button>}
            />
          )}
        </div>
      </div>
      ...
      <div className="rounded-[6px] border border-border bg-surface overflow-hidden">
        ...
        <div className="flex border-b border-border px-4 gap-1 pt-1">
          <Link href="/transactions" className="px-3 py-2 text-[13px] text-muted-foreground hover:text-foreground transition-colors">
            All Transactions
          </Link>
          <span className="px-3 py-2 text-[13px] font-medium text-foreground border-b-2 border-primary">
            Income
          </span>
          <Link href="/transactions/expenses" className="px-3 py-2 text-[13px] text-muted-foreground hover:text-foreground transition-colors">
            Expenses
          </Link>
        </div>
          ...
          <table className="w-full text-[13px] border-collapse">
            ...
            <tbody>
              {rowsWithDisplay.length === 0 ? (
                <tr>
                  <td colSpan={canEdit ? 8 : 7} className="px-5 py-16 text-center text-[13px] text-muted-foreground">
                ...
                rowsWithDisplay.map((t) => {
                  ...
                  return (
                    <tr key={t.id} className="border-b border-border/50 hover:bg-elevated/40 transition-colors last:border-0">
                      ...
                      <td className="px-4 py-2.5 text-right tabular-nums font-medium text-income">
            ...
            <div className="flex items-center gap-1">
              {page > 1 && (
                <Button asChild variant="ghost" size="sm">
                  <Link href={`/transactions/income?page=${page - 1}`}>&larr;</Link>
                </Button>
              )}
              {[1, 2, 3, 4, 5].filter(p => p <= totalPages).map((p) => (
                <Button key={p} asChild={p !== page} variant={p === page ? "default" : "ghost"} size="sm">
                  {p === page ? <span>{String(p)}</span> : <Link href={`/transactions/income?page=${p}`}>{p}</Link>}
                </Button>
              ))}
              {totalPages > 5 && <span className="px-1 text-muted-foreground text-[12px]">...</span>}
              {totalPages > 5 && (
                <Button asChild variant="ghost" size="sm">
                  <Link href={`/transactions/income?page=${totalPages}`}>{totalPages}</Link>
                </Button>
              )}
              {page < totalPages && (
                <Button asChild variant="ghost" size="sm">
                  <Link href={`/transactions/income?page=${page + 1}`}>&rarr;</Link>
                </Button>
              )}
            </div>
          )}

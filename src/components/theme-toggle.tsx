    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        ...
        >
          ...
          <ChevronDown className="size-3 opacity-50 mt-px" />
        </button>
      </DropdownMenuTrigger>
        ...
        {OPTIONS.map(({ value, label, Icon }) => {
          ...
          return (
            <DropdownMenuItem
              ...
              className={active ? "text-primary font-medium bg-primary/5" : ""}
            >
              <Icon className="mr-2 size-4 shrink-0" />
              <span className="flex-1">{label}</span>
              {active && (
                <span className="ml-2 size-1.5 rounded-full bg-primary inline-block" />
              )}
            </DropdownMenuItem>
          );
        })}

...
import {
  User, Building2, Users, Shield, Bell, Sliders,
} from "lucide-react";
...
const NAV_ITEMS = [
  ...
  { href: "/settings/organization",  label: "Organization",  icon: Building2, guard: canManageOrganization },
...
export function SettingsNav({ role }: SettingsNavProps) {
  ...
  return (
    <nav className="flex flex-row flex-wrap gap-1 sm:w-44 sm:shrink-0 sm:flex-col sm:flex-nowrap sm:gap-0.5">
      {items.map((item) => {
        ...
        return (
          ...
          >
            ...
            <Icon
              className={cn(
                "size-[14px] shrink-0",
                active ? "text-sidebar-active-foreground/80" : "text-muted-foreground/50 group-hover:text-muted-foreground"
              )}
            />
            {item.label}
          </Link>
